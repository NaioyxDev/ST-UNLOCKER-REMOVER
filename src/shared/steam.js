  ...
  try {
    ...
    return {
      ...
      genres: Array.isArray(entry.data.genres)
        ? entry.data.genres.map((g) => g.description)
        : []
    };
  } catch {
    return null;
...
async function scanSteamUnlockers(steamExePath) {
  ...
  try {
    ...
    if (error.code === 'ENOENT') {
      return {
        ...
        pluginDir,
        unlockers: [],
        summary: { totalFiles: 0, totalSize: '0 B' }
      };
    }
    throw error;
  }

  const luaFiles = entries.filter((e) => e.isFile() && e.name.toLowerCase().endsWith('.lua'));
  ...
  const totalBytes = unlockers.reduce((acc, e) => acc + e.size, 0);
  return {
    steamExePath,
    pluginDir,
    unlockers,
